...
export const QualityTab: React.FC<QualityTabProps> = ({
  ...
  if (loading) {
    return (
      <Card className="border-slate-200">
        <CardContent className="p-12 text-center text-slate-400 text-xs flex items-center justify-center gap-2">
  ...
  if (error || !quality) {
    return (
      <Card className="border-slate-200">
        <CardContent className="p-8 text-center text-rose-600 text-xs">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-2xs">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div className="w-24 h-24 rounded-2xl bg-slate-50 border-2 border-indigo-100 flex flex-col items-center justify-center shadow-inner">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-slate-900 font-mono">{dim.score}%</span>
              </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-2xs">
        ...
        <div className="divide-y divide-slate-100">
          {quality.rules.map((rule, idx) => {
            ...
            return (
              <div key={idx} className="p-3.5 hover:bg-slate-50/70 transition">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-600">{rule.explanation}</p>
                  </div>

                  <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
                    <span className="text-xs text-slate-500 font-mono">
                      Pogođeno: <strong>{rule.affectedRecordCount}</strong> (
                  ...
                  <div className="mt-3 p-3 bg-slate-50 rounded-lg border border-slate-200 text-xs space-y-2">
                    ...
                    <div className="space-y-1.5">
                      {rule.examples.map((ex, exIdx) => (
                        <pre
                          key={exIdx}
                          className="p-2 bg-white border border-slate-200 rounded text-[11px] font-mono overflow-x-auto text-slate-800"
                        >
