...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-2xs flex flex-col h-[650px] overflow-hidden">
      {/* Chat Header */}
      <div className="px-5 py-3.5 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/40 flex items-center justify-between">
        <div className="flex items-center space-x-2.5">
          <div className="p-2 bg-indigo-600 dark:bg-indigo-500 rounded-lg text-white shadow-2xs">
            <Bot className="w-4 h-4" />
          </div>
          <div>
            <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5">
              FAP Data Assistant
              <span className="text-[10px] font-mono bg-indigo-100 dark:bg-indigo-950/60 text-indigo-800 dark:text-indigo-300 px-1.5 py-0.2 rounded">
                Gemini Flash
              </span>
            </h3>
            <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
              Kontekst: <span className="font-medium text-zinc-700 dark:text-zinc-300">{datasetName} (v{versionNumber})</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="flex flex-wrap gap-1.5 justify-center max-w-lg mx-auto">
              {EXAMPLE_PROMPTS.map((prompt, i) => (
                <button
                  ...
                  className="text-[11px] bg-zinc-100 dark:bg-zinc-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 hover:text-indigo-700 dark:hover:text-indigo-300 text-zinc-700 dark:text-zinc-300 px-2.5 py-1 rounded-full border border-zinc-200 dark:border-zinc-700 transition-colors"
                >
                  {prompt}
                </button>
              ))}
        ...
        {messages.map((m, idx) => {
          ...
          return (
            ...
            >
              {!isUser && (
                <div className="w-7 h-7 rounded-full bg-indigo-100 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-400 flex items-center justify-center shrink-0 text-xs font-bold mt-0.5">
                  <Bot className="w-4 h-4" />
                </div>
              )}
              ...
              <div
                className={`max-w-[85%] sm:max-w-xl rounded-xl p-3.5 text-xs ${
                  isUser
                    ...
                    : "bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 rounded-tl-none"
                }`}
              >
                ...
                {!isUser && m.isDemoFallback && (
                  <div className="mt-2 text-[10px] text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded px-2 py-1 flex items-center gap-1">
                ...
                {!isUser && m.chart && m.chart.data && m.chart.data.length > 0 && (
                  <div className="mt-3 bg-white dark:bg-zinc-900 p-2.5 border border-zinc-200 dark:border-zinc-800 rounded-lg">
                    <div className="text-[11px] font-semibold text-zinc-800 dark:text-zinc-200 mb-2 flex items-center gap-1">
              ...
              {isUser && (
                <div className="w-7 h-7 rounded-full bg-zinc-800 dark:bg-zinc-700 text-white flex items-center justify-center shrink-0 text-xs font-bold mt-0.5">
        ...
        {loading && (
          <div className="flex items-center space-x-2 text-xs text-zinc-500 dark:text-zinc-400 pl-9">
      ...
      {/* Error state */}
      {error && (
        <div className="px-4 py-2 bg-rose-50 dark:bg-rose-950/40 border-t border-rose-200 dark:border-rose-900 text-rose-700 dark:text-rose-300 text-xs flex items-center justify-between">
          <span>{error}</span>
      ...
      <div className="p-3 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800">
        <div className="flex items-center space-x-2">
          <input
            ...
            disabled={loading}
            className="flex-1 text-xs sm:text-sm px-3.5 py-2.5 bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none disabled:bg-zinc-50 dark:disabled:bg-zinc-800"
          />
          <Button
            onClick={() => handleSend()}
